...
```
✂️ **Bonus:** la función problemática `safe_label_transform` fue **eliminada** (resuelve también W12).

---

### 4. ✅ `inverse_transform` repetido ×3 — ahora `:318`
...
### 10. ⚠️ Feedback hardcodeado — `:339-347` — **PENDIENTE** 🔵
...
|--------|-------|------------------|--------------|
| ✅ | C1 — Split separador | `split(';')` | `:42` |
| ✅ | C2 — fillna masivo | Imputación separada numérica/categórica | `:20-24` |
| ✅ | C3 — collect_feedback corrupto | Preprocesar `new_row` antes de concatenar | `:226-248` |
| ✅ | C4 — inverse_transform triplicado | Una sola llamada + desempaquetar | `:318` |
| ✅ | C5 — time.sleep(30) final | Línea eliminada | — |
| ✅ | C6 — time.sleep(5) TTS | Eliminado + bucle `iterate()` p/ Python 3.13 | `:66-74` |
| ✅ | M7 — Modelo no guardado | `save()`/`load_model` con `modelo5.keras` | `:264, 296-299, 350` |
| ✅ | M8 — Split sin stratify | Stratify fase1+fase2 + reintroducir clases únicas | `:276-294` |
| ✅ | M9 — Nombres fase en inglés | Traducidos a español | `:210-217, 241` |
| 🔵 | W10 — Feedback hardcodeado | **Pendiente**: ventana de diálogo real | `:339-347` |
| ✅ | W11 — output_dims duplicado | Inline como kwarg (sin variable local) | `:255` |
| ✅ | W12 — print() vacío | `safe_label_transform` eliminada | — |
| ✅ | W13 — `import time;` | Eliminado el `;` | `:10` |
